package com.sc.zhaoqi.web.system;
...
public final class SystemRequirements
{
    ...
    private static void verify64BitJvm()
    {
        ...
        if (!"64".equals(dataModel)) {
            failRequirement("AirliftWeb requires a 64-bit JVM (found %s)", dataModel);
        }
    }
    ...
    private static void verifyByteOrder()
    {
        ...
        if (!order.equals(ByteOrder.LITTLE_ENDIAN)) {
            failRequirement("AirliftWeb requires a little endian platform (found %s)", order);
        }
    }
    ...
    private static void verifyOsArchitecture()
    {
        ...
        if ("Linux".equals(osName)) {
            if (!"amd64".equals(osArch) && !"ppc64le".equals(osArch)) {
                failRequirement("AirliftWeb requires amd64 or ppc64le on Linux (found %s)", osArch);
            }
            ...
            if (!"x86_64".equals(osArch)) {
                failRequirement("AirliftWeb requires x86_64 on Mac OS X (found %s)", osArch);
            }
        }
            ...
            failRequirement("AirliftWeb requires Windows or Linux or Mac OS X (found %s)", osName);
        }
    }
    ...
    private static void verifyJavaVersion()
    {
        ...
        failRequirement("AirliftWeb requires Java 8u151+ (found %s)", javaVersion);
    }
    ...
    private static void verifyUsingG1Gc()
    {
        try {
            ...
            if (garbageCollectors.stream().noneMatch(name -> name.toUpperCase(Locale.US).startsWith("G1 "))) {
                warnRequirement("Current garbage collectors are %s. AirliftWeb recommends the G1 garbage collector.", garbageCollectors);
            }
        }
    ...
    private static void verifyFileDescriptor()
    {
        ...
        if (maxFileDescriptorCount.getAsLong() < MIN_FILE_DESCRIPTORS) {
            failRequirement("AirliftWeb requires at least %s file descriptors (found %s)", MIN_FILE_DESCRIPTORS, maxFileDescriptorCount.getAsLong());
        }
    ...
    private static OptionalLong getMaxFileDescriptorCount()
    {
        try {
            ...
            // for windows we couldn't get it,so simple set it to
            return OptionalLong.of(Runtime.getRuntime().availableProcessors() * 5000);
        }
    }
    ...
    public static void verifySystemTimeIsReasonable()
    {
        ...
        if (currentYear < 2015) {
            failRequirement("AirliftWeb requires the system time to be current (found year %s)", currentYear);
        }
    }
